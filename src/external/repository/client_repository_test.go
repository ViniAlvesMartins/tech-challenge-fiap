...
func TestClientRepository_GetClientById(t *testing.T) {
	t.Run("get client by id successfully", func(t *testing.T) {
		...
		expectedSQL := `SELECT * FROM "clients" WHERE id=$1 ORDER BY "clients"."id" LIMIT 1`
		mock.ExpectQuery(regexp.QuoteMeta(expectedSQL)).WithArgs(id).WillReturnRows(rows)
...
func TestClientRepository_GetAlreadyExists(t *testing.T) {
	t.Run("get client successfully", func(t *testing.T) {
		...
		expectedSQL := `SELECT * FROM "clients" WHERE cpf=$1 OR email=$2 ORDER BY "clients"."id" LIMIT 1`
		mock.ExpectQuery(regexp.QuoteMeta(expectedSQL)).WithArgs(12345678900, "testclient@example.com").WillReturnRows(rows)
		client, err := repo.GetAlreadyExists(12345678900, "testclient@example.com")
	...
	t.Run("database error", func(t *testing.T) {
		...
		expectedSQL := `SELECT * FROM "clients" WHERE cpf=$1 OR email=$2 ORDER BY "clients"."id" LIMIT 1`
		mock.ExpectQuery(regexp.QuoteMeta(expectedSQL)).WithArgs(12345678900, "testclient@example.com").WillReturnError(expectedErr)
		client, err := repo.GetAlreadyExists(12345678900, "testclient@example.com")
