...
func main() {
	...
	paymentStatusHandler := sqs.NewPaymentStatusUpdateHandler(ordersUseCase, logger)

	logger.Info("Starting consumer...")
	failedPaymentConsumer := sqs.NewConsumer(consumer, paymentStatusHandler, logger)

	var wg sync.WaitGroup
	wg.Add(1)
	go failedPaymentConsumer.Start(ctx, &wg)
